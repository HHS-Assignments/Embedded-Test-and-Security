...
//code voor practicum 2 C security
struct orderregel {
	char idnr[8];
	short aantal;
};
...
void test_functie() {
	printf("Demo voor aangepast return adres");
}
...
void malloc_voorbeeld() {
	char* p = (char*)malloc(16);
	strcpy(p, "Inhoud van p");
	char* q = (char*)malloc(16);
	strcpy(q, "Inhoud van q");
	printf("Adres p: %p, Adres q: %p Verschil (in dec) %d \n", p, q, p - q);
}
int main(int argc, char** argv) {
	...
	while (fscanf(f, " %s %d", artikelnr, &aantal) == 2) {
		...
		printf("%s %d\n", artikelnr, aantal);
		i++;
		bestelling[i].aantal = aantal;
		strcpy(bestelling[i].idnr, artikelnr);
	}
	...
	if (!klant_geverifieerd) {
		...
		int prijs = berekenPrijsPracticum2(bestelling, i);
		printf("Order ok\n");
